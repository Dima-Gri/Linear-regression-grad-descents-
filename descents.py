...
class BaseDescent:
    ...
    """
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE, gamma: float = 1.0):
        """
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function == LossFunction.MSE:
            ...
            return diff.T @ diff / y.shape[0]
        if self.loss_function == LossFunction.LogCosh:
            diff = self.predict(x) - y
            return np.sum(np.log(np.cosh(diff))) / y.shape[0]
        if self.loss_function == LossFunction.Huber:
            diff = self.predict(x) - y
            abl = np.abs(diff)
            return np.mean(np.where(abl <= self.gamma, (diff)**2 / 2, self.gamma * (abl - self.gamma / 2)))
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        diff = self.predict(x) - y
        if self.loss_function == LossFunction.MSE:
            return 2 * x.T @ diff / y.shape[0]
        if self.loss_function == LossFunction.LogCosh:
            return x.T @ np.tanh(diff) / y.shape[0]
        if self.loss_function == LossFunction.MAE:
            return x.T @ np.sign(diff) / x.shape[0]
        if self.loss_function is LossFunction.Huber:
            abl = np.abs(diff)
            return (x[abl <= self.gamma].T @ diff[abl <= self.gamma] + self.gamma * x[abl > self.gamma].T @ np.sign(diff[abl > self.gamma])) / x.shape[0]
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, batch_size: int = 50,
                 loss_function: LossFunction = LossFunction.MSE, gamma: float =1.0):
        """
        :param batch_size: batch size (int)
        """
        super().__init__(dimension, lambda_, loss_function, gamma)
        self.batch_size = batch_size

    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        batch = np.random.randint(0, x.shape[0], size=self.batch_size)
...
class Nadam(Adam):        
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_hat = self.beta_1 * self.m + (1 - self.beta_1) / (1 - self.beta_1 ** self.iteration) * gradient
        v_hat = self.v / (1 - self.beta_2 ** self.iteration)
